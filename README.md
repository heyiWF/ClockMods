...
- 中英文之间、中文与数字、数字与单位之间按书写规范自动加入半角空隙。
...
- 由 [和风天气（QWeather）](https://www.qweather.com) 提供数据，请求使用 Ed25519（EdDSA）签名鉴权。
- 支持自动定位或手动指定省/市/区县；天气随界面语言显示为中文或英文。
- 时间下方显示当前地点、天气状况、温度与对应的 QWeather 天气图标。
...
- 天气仅在应用前台定位与刷新；定位、网络或配置失败时在天气行显示状态提示。
...
专业版还可定制天气图标：在 **填充（fill）** 与 **线条（line）** 两种风格之间切换，并可选用 Material 3 动态主题色为图标着色（关闭时为白色）。
...
三个版本共享时钟、农历、网络校时、时区、设置存储与背景图片处理逻辑；专业版复用现代版的界面与资源，并叠加自身的工具页面与后台调度。Material Components 仅用于现代版与专业版，因此不会抬高兼容版的最低系统版本。界面语言切换后整个应用（设置、月历、工具页、天气等）随之切换，农历始终以中文显示。
...
ClockMods 目前不提供后台天气更新、桌面小部件、云同步与图片编辑功能。闹钟、倒计时、番茄钟、秒表、整点报时与月历仅在专业版中提供。
