...
- 可进一步开启“显示详细天气信息”，在时间下方轮播体感温度、相对湿度、风向风力和气象灾害预警等；单条信息过宽时横向滚动显示，不换行也不拆分。
...
- **日历**：以仪表盘形式按月查看公历日历，支持左右滑动或前后翻月、点选日期并一键回到今天。日历离线整合节日、二十四节气与法定节假日“休/班”标记，卡片底部以上滑轮播依次展示所选日期、当日“宜”（绿色整行）与“忌”（红色整行），单行过宽时横向滚动、不换行也不拆分。横屏时左侧同时显示时间、实时天气（含体感温度、风向与湿度）以及今天、明天、后天的三日预报。
- **番茄钟**：基于计时器的番茄工作法计时。
...
| 专业版 `pro` / ClockMods Pro | Android 12（API 31） | `com.clockmods.pro` | 在现代版界面基础上，增加闹钟、倒计时、番茄钟、秒表、日历仪表盘和整点报时等工具页面 |
...
专业版日历的农历、二十四节气、传统与公历节日、数九/三伏以及每日“宜/忌”，均由离线农历引擎 [lunar-java](https://github.com/6tail/lunar-java)（`cn.6tail:lunar`，MIT 许可，无第三方依赖）实时计算。它采用香港天文台阴阳合历与天文节气算法（非寿星公式经验值），可准确覆盖到 2100 年以后，因此无需再内置 ICS 日历数据；宜忌依据《钦定协纪辨方书》神煞宜忌规则推算。

月历默认只显示主要节日；在“功能”设置中开启“月历显示更多节日”后，会一并显示地球日、护士节等更多小众纪念日。单元格中超过三个字且放不下的节日会像宜忌一样横向滚动显示，三个字及以内的保持居中不滚动。

法定节假日“休/班”安排无法通过算法推算（每年由国务院公布），继续使用随应用打包的离线数据 [holiday-cn](https://github.com/NateScarlet/holiday-cn)（MIT 许可，见 `app/src/pro/assets/holidays` 与 `app/src/pro/assets/licenses`）。
