...
- 支持中文或英文界面语言，切换后整个应用（设置面板、月历、工具页、天气等）随之切换；农历始终保持中文显示。
- 可在设置中切换日期格式：专业版提供完整的中/英文日期格式与星期组合自由搭配，并支持用表达式与自定义分隔符（含 emoji）自定义格式；兼容版与现代版提供一组固定日期格式选项。日期格式区分中英文，示例统一按 2026 年 8 月 7 日星期五预览。
- 可设置自定义留言，非空时显示在时钟界面的天气行；与天气同时启用则轮播天气与留言，单行过宽时横向滚动。
- 中英文之间、中文与数字之间以及数字与计量单位之间按书写规范加入半角空隙。
...
- 可在“功能”页开启实时天气；天气随界面语言显示为中文或英文（英文界面下通过 QWeather 的英文接口获取天气状况与预警文案）。
