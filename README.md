...
5. 点击“应用”保存设置；选择的背景图片会复制到应用私有目录中。
...
两个版本共享时钟、农历、网络校时、时区、设置存储和背景图片处理逻辑。Material Components 仅用于现代版，因此不会提高兼容版的最低 Android 版本。
...
复制 `qweather.properties.example` 为 `qweather.properties`，填写 API Host、凭据 ID、项目 ID 和 PKCS#8 Ed25519 私钥的 Base64 内容。真实配置文件已加入 `.gitignore`，不得提交到版本库。

客户端直连意味着私钥最终会进入 APK，本地忽略文件只能防止误提交，不能防止 APK 逆向提取。公开分发前建议改为后端代理或短期 Token 服务。聊天、日志或其他渠道中暴露过的凭据应立即在 QWeather 控制台轮换。

天气数据由 [和风天气](https://www.qweather.com) 提供。QWeather Icons 来自随项目导入的 QWeather Icons 1.8.0，图标采用 CC BY 4.0 许可。

## 当前不包含的功能

ClockMods 当前不提供闹钟、倒计时、后台天气更新、桌面小部件、云同步和图片编辑功能。
...
```powershell
.\gradlew.bat testCompatDebugUnitTest testModernDebugUnitTest
```

构建两个版本的 Debug APK：

```powershell
.\gradlew.bat assembleCompatDebug assembleModernDebug
```

运行 Lint 检查：

```powershell
.\gradlew.bat lintCompatDebug lintModernDebug
```
...
```text
app/src/main/    两个版本共享的时钟、农历、时间、背景和资源代码
